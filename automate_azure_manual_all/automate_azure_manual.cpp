...
int input[12]={atoi(argv[1]), atoi(argv[2]), atoi(argv[3]), atoi(argv[4]), atoi(argv[5]), atoi(argv[6]), atoi(argv[7]), atoi(argv[8]), atoi(argv[9]), atoi(argv[10]), atoi(argv[11]), atoi(argv[12])};
std::string naming = std::string("auto")+ *argv[1]+"_" + *argv[2] + "_" + *argv[3] + "_" + *argv[4] + "_" +*argv[5] +"_" + *argv[6]+"_" + *argv[7] + "_" + *argv[8] + "_" + *argv[9]+ "_" +*argv[10]+"_"+ *argv[11]+"_" + *argv[12];
...
double RESON[13]={6.950, 7.041, 7.176, 7.370, 7.480, 7.801, 8.026, 8.189, 8.322, 8.507, 8.586, 8.672,8.717};
...
outfile<< "/home/amber/Documents/Tech/azure/outputs/manual/all_12/"<<                              "#Full Path to Output Directory" <<endl;
outfile<< "/home/amber/Documents/Tech/azure/outputs/manual/all_12/"<<	                           "#Full Path to Checks Directory" <<endl;
...
for(a=0; a<12; a++){	
	...
	for (c=0; c<JPI[input[a]][2]; c++){
		...
		// everything that's not a user variable is fixed for this experimental setup but you need to change it if you change experiments.
		outfile << JPI[input[a]][0] << "\t" << JPI[input[a]][1] <<  "\t" <<   RESON[a]   <<"\t1      1       1\t"<< ELLE << "\t" <<   JPI[input[a]][c+3] << "\t" <<  a+1 << "\t1       0\t"   <<   PARTIAL[input[a]][c]<< "\t0.5     1       1.5     1       0       1       37      1       19      4.547   4.547   0       0       0.0     0       6.015 0       0       0\t" << endl;
...
outfile << "1              1              1              2.5            4.3            134.58         134.58         1              1              0              0	/home/amber/ownCloud/Documents/Tech/azure/37K_new/qqq3_11_27_134.58.dat" << endl;
outfile << "1              1              1              2.5            4.3            138.9          138.9          1              1              0              0	/home/amber/ownCloud/Documents/Tech/azure/37K_new/qqq5_11_27_138.9.dat" << endl;
...
outfile << "\"/home/amber/Documents/Tech/azure/outputs/manual/all_12/"<<naming<<".out\"" << endl;
outfile << "\"/home/amber/Documents/Tech/azure/outputs/manual/all_12/"<<naming<<".extrap\"" << endl;
outfile << "0 \"\" "<< endl;
outfile << "-1. -1. -1." << endl;
outfile << "</lastRun>" << endl;
